...
def compute_mcmc_stats(x, L=20):
    ...
    return [mean, np.sqrt(var / ess), np.sqrt(var), ess]
...
def target_lpdf(x):
    return - 0.5 * sum([x_n ** 2 for x_n in x]) \
           - 0.5 * len(x) * np.log(6.283185307179586)
...
for n in range(1, N):
    ...
    if target_lpdf(xp) < target_lpdf(x0):
        accept_prob = np.exp(target_lpdf(xp) - target_lpdf(x0))
...
def hamiltonian(q, p):
    return 0.5 * np.dot(p, p) - target_lpdf(q)


# Tuning
T = 1.8 * np.pi
epsilon = 0.01
L = int(np.floor(T / epsilon))

# Visualize a single trajectory
q = np.asarray([1.0 for _ in range(D)])
p = np.asarray([stats.norm.rvs(0, 1) for _ in range(D)])
...
for n in range(1, N):
    ...
    for l in range(Lp):
        ...
        sum_metro_prob += min(1, np.exp(H0 - H))

        u = stats.uniform.rvs(0, 1)
        if u < np.exp(log_weight - log_sum_weight):
            q_propose = q
    ...
    for l in range(L - 1 - Lp):
        ...
        u = stats.uniform.rvs(0, 1)
        if np.exp(log_weight - log_sum_weight):
            q_propose = q
